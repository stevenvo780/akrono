          ...
          {lines.map((line) => {
            ...
            return (
              <div key={line.slug} className="card p-4 flex gap-4 items-center">
                ...
                <img src={`/api/img/${p.slug}`} alt={pName(p, l)} className="w-20 h-20 rounded-lg object-cover" />
                <div className="flex-1 min-w-0">
                  <Link href={`/producto/${p.slug}`} className="font-display font-semibold hover:text-[var(--clay)] line-clamp-1">
                    {pName(p, l)}
                  </Link>
                  <div className="text-sm text-[var(--clay)] font-semibold mt-1">
                    {price(p.price_cop, p.price_usd, l)}
                  </div>
                </div>
