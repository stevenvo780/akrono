...
import { storePath } from "@/components/StoreLink";
...
export default function CheckoutPage() {
  ...
  const shipping =
    scope === "internacional"
      ? ship.international_usd
      : subtotalCOP >= ship.free_national_over_cop
        ? 0
        : ship.flat_national_cop;
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const items = lines.map((li) => ({
      slug: li.slug,
      name: l === "en" ? li.name_en : li.name_es,
      qty: li.qty,
      price_cop: li.price_cop,
    ...
    const res = await fetch(`/api/orders?store=${store.slug}`, {
      method: "POST",
    ...
    if (res.ok) {
      ...
      clear();
      router.push(storePath(store.slug, `/pedido/${id}`));
    } else {
      setSubmitting(false);
            ...
            {lines.map((li) => (
              <div key={li.slug} className="flex justify-between gap-2">
                <span className="text-neutral-600 line-clamp-1">
                  {li.qty}× {l === "en" ? li.name_en : li.name_es}
                </span>
                <span className="font-medium whitespace-nowrap">
                  {money((currency === "USD" ? li.price_usd : li.price_cop) * li.qty, currency)}
                </span>
              </div>
            ))}
          </div>
