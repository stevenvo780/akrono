...
export async function generateMetadata({
  ...
  return {
    title,
    description,
    openGraph: { title, description, images: [`/api/img/${p.slug}?store=${store}`], type: "website" },
  };
}

export default async function ProductPage({ params }: { params: Promise<{ store: string; slug: string }> }) {
  const { store, slug } = await params;
  ...
  const cat = getCategory(store, product.category);
  const related = productsByCategory(store, product.category)
    ...
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      <div className="grid md:grid-cols-2 gap-10">
        <div className="card overflow-hidden">
          ...
          <img src={`/api/img/${product.slug}?store=${store}`} alt={pName(product, l)} className="w-full aspect-square object-cover" />
        </div>
