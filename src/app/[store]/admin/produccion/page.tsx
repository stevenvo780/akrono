...
export default async function ProduccionPage({ params }: { params: Promise<{ store: string }> }) {
  const { store } = await params;
  const states = listProductStates(store);
  const queue = states
    .filter((s) => s.production_status !== "terminado" || s.in_production > 0)
    .sort((a, b) => b.in_production - a.in_production);
  const byslug = new Map(getProducts(store).map((p) => [p.slug, p]));
